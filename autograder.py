...
def test_min_fill_ordering():
    print("\nMin Fill Ordering Test ... ", end='')

    # Constructing a Bayesian Network
    E, B, S, G, W = Variable('E', ['e', '-e']), Variable('B', ['b', '-b']), Variable('S', ['s', '-s']), \
                    Variable('G', ['g', '-g']), Variable('W', ['w', '-w'])
    FE, FB, FS, FG, FW = Factor('P(E)', [E]), Factor('P(B)', [B]), Factor('P(S|E,B)', [S, E, B]), \
                         Factor('P(G|S)', [G, S]), Factor('P(W|S)', [W, S])

    FE.add_values([['e', 0.1], ['-e', 0.9]])
    FB.add_values([['b', 0.1], ['-b', 0.9]])
    FS.add_values([['s', 'e', 'b', 0.9], ['s', 'e', '-b', 0.2], ['s', '-e', 'b', 0.8], ['s', '-e', '-b', 0],
                   ['-s', 'e', 'b', 0.1], ['-s', 'e', '-b', 0.8], ['-s', '-e', 'b', 0.2], ['-s', '-e', '-b', 1]])
    FG.add_values([['g', 's', 0.5], ['g', '-s', 0], ['-g', 's', 0.5], ['-g', '-s', 1]])
    FW.add_values([['w', 's', 0.8], ['w', '-s', 0.2], ['-w', 's', 0.2], ['-w', '-s', 0.8]])

    ExampleBN = BN('ExampleBN', [E, B, S, G, W], [FE, FB, FS, FG, FW])

    # Calling min_fill_ordering
    ordering = min_fill_ordering([FB, FE, FS, FG, FW], G)

    # Checking the ordering
    expected_ordering = ['W', 'B', 'E', 'S']
    if ordering == expected_ordering:
        print("passed.")
    else:
        print("failed.")
        print("Expected Ordering:", expected_ordering)
        print("Actual Ordering:", ordering)


if __name__ == '__main__':
    if test_multiply:
        test_multiply_fun()
    min_fill_ordering([FE, FB, FS, FG],G)
